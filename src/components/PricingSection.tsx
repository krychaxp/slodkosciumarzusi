...
import type { PricingItem } from '../data/content'
...
function PricingCard({ emoji, heading, subtitle, colNr, colName, colPrice, items, note }: PricingCardProps) {
  return (
          ...
          <tbody>
            {items.map((item, i) => (
              <tr key={i} className="hover:bg-[#fff9f9] border-b border-[#eee] last:border-b-0">
                ...
                <td className="py-2.5 px-4 text-[#555] text-[0.95rem]">{item.name}</td>
                <td className="py-2.5 px-4 text-right font-semibold text-[#e17055] text-[0.95rem] whitespace-nowrap">{item.price}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function PricingSection() {
  ...
  const dessertItems = t.raw('pricing.dessertItems') as PricingItem[]
  const cakeItems = t.raw('pricing.cakeItems') as PricingItem[]
    ...
    >
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid gap-8 mt-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))' }}>
          <PricingCard
            emoji="🍮"
            heading={t('pricing.dessertsHeading')}
            subtitle={t('pricing.dessertsSubtitle')}
            colNr={t('pricing.colNr')}
            colName={t('pricing.colName')}
            colPrice={t('pricing.colPrice')}
            items={dessertItems}
            note={
              <>
                <strong className="text-[#5c3d4a]">{t('pricing.dessertsNoteLabel')}:</strong>{' '}
                {t('pricing.dessertsNoteValue')}
              </>
            }
          />
          <PricingCard
            emoji="🍰"
            heading={t('pricing.cakesHeading')}
            subtitle={t('pricing.cakesSubtitle')}
            colNr={t('pricing.colNr')}
            colName={t('pricing.colName')}
            colPrice={t('pricing.colPrice')}
            items={cakeItems}
            note={
              <>
                {t('pricing.cakesNote')}
                <br /><br />
                <strong className="text-[#5c3d4a]">{t('pricing.tortesNoteLabel')}:</strong>{' '}
                {t('pricing.tortesNoteValue')}
              </>
            }
          />
        </div>
